...
export const metadata: Metadata = {
  title: 'Investment Solutions - Beart India Group',
  description: 'Comprehensive investment solutions including Mutual Funds, Equity, AIF, PMS, NCDs, Smallcase, Insurance, Bonds, FDs, and Loans. Expert financial advisory by Beart India Group.',
...
export default function InvestmentSolutionsPage() {
  return (
    <div className="container mx-auto flex-grow py-12 md:py-20 px-4 md:px-6 animate-in fade-in duration-500 w-full">
      
      <Card className="mx-auto shadow-lg w-full border-0 shadow-none"> 
        ...
        <CardContent className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
          ...
          <div className="prose prose-sm sm:prose-base lg:prose-lg max-w-none mx-auto text-muted-foreground space-y-6">
            ...
            <h2 className="text-xl sm:text-2xl font-semibold text-foreground !mt-10 !mb-4 text-center md:text-left">Our Core Services</h2>
                ...
                {investmentServices.map((service, index) => (
                  <Link key={service.title} href={service.link} className="block hover:shadow-md transition-shadow duration-200 rounded-lg group">
                     <div className="p-4 border rounded-lg bg-card h-full flex flex-col animate-in fade-in zoom-in-95 duration-500 text-left" style={{ animationDelay: `${index * 100}ms` }}>
                       <h3 className="flex items-center gap-2 font-semibold text-base sm:text-lg text-foreground mb-2 group-hover:text-primary">
                       ...
                       <p className="text-xs sm:text-sm text-muted-foreground flex-grow mb-3">
                        ...
                        <span className="text-primary font-medium text-xs sm:text-sm mt-auto">Learn More <ArrowRight className="inline-block ml-1 h-3 w-3 sm:h-4 sm:w-4" /></span>
                     </div>
                  </Link>
                ))}
             </div>

             <h2 className="text-xl sm:text-2xl font-semibold text-foreground !mt-10 !mb-4 text-center md:text-left">Why Choose Beart India Investments?</h2>
