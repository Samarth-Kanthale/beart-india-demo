...
const leadFormSchema = z.object({
  ...
  phone: z.string()
    .optional()
    .refine(val => {
      if (val === undefined || val.trim() === "") return true; // Optional and empty is fine
...
export function LeadGenerationForm({ serviceTitle, className }: LeadGenerationFormProps) {
  ...
  async function onSubmit(values: LeadFormValues) {
    ...
    // Pass loanAmount as a string if it exists, server action will handle conversion
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle>Interested in {serviceTitle}?</CardTitle>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                       {/* Use type="text" and inputMode="numeric" for better mobile UX for numbers */}
                      <Input 
                        type="text" 
                        inputMode="numeric"
                        pattern="[0-9]*"
                        ...
                        {...field}
                        onChange={event => field.onChange(event.target.value)} // Pass string value
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder={`Tell us more about your requirements for ${serviceTitle}...`} rows={4} {...field} disabled={isSubmitting} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
